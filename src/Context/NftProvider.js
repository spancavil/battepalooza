    ...
    (async () => {
      try {
        ...
        if (response?.error.num === 0) {
          response.mainDrop.bigBannerUrl = changeDropUrl(
            BP_BASE_URL,
            response.mainDrop?.bigBannerUrl
          );
          response.mainDrop.mediumBannerUrl = changeDropUrl(
            BP_BASE_URL,
            response.mainDrop?.mediumBannerUrl
          );
          response.mainDrop.smallBannerUrl = changeDropUrl(
            BP_BASE_URL,
            response.mainDrop?.smallBannerUrl
          );
          setDrops(response);
        }
      } catch (error) {
